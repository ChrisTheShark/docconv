...
func ConvertPDFText(path string) (BodyResult, MetaResult, error) {
	...
	go func() {
		body, err := exec.Command("pdftotext", "-q", "-layout", "-nopgbrk", "-enc", "UTF-8", "-eol", "unix", path, "-").Output()
		if err != nil {
			bodyResult.err = err
		}
...
// Convert PDF
func ConvertPDFTextWithPage(path string, start, end int) (BodyResult, MetaResult, error) {
	metaResult := MetaResult{meta: make(map[string]string)}
	bodyResult := BodyResult{}
	mr := make(chan MetaResult, 1)
	go func() {
	...
	br := make(chan BodyResult, 1)
	go func() {
		body, err := exec.Command("pdftotext", "-q", "-layout", "-nopgbrk", "-enc", "UTF-8", "-eol", "unix",
			"-f", fmt.Sprintf("%d", start), "-l", fmt.Sprintf("%d", end), path, "-").Output()
		if err != nil {
			bodyResult.err = err
		}
